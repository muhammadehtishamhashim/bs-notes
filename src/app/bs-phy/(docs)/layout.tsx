...
import { bsPhySource } from '@/lib/source'
    ...
    <DocsLayout
      ...
      nav={{
        ...
        title: (
          <>
            {logo}
            <span className='font-medium max-md:hidden'>BS PHYSICS NOTES</span>
          </>
        ),
      }}
      sidebar={{
        ...
        tabs: {
          transform(option, node) {
            const meta = bsPhySource.getNodeMeta(node)
            if (!(meta && node.icon)) {
              return option
            }
